...
# Load the data
consumer_tweet_path = '/Users/kamrul.konok/Desktop/sdm_joint_project/data/consumer_tweet.csv'
...
def tweets():
    for index, row in consumer_tweet.iterrows():
        ...
        brand_uri = URIRef(f"http://example.org/digiscan360/brand/{quote(str(row['brand_name']))}")

        # Add tweet instance
        g.add((tweet_uri, RDF.type, ds.Tweet))
        g.add((tweet_uri, ds.tweet_id, Literal(row['tweet_id'], datatype=XSD.string)))
        g.add((tweet_uri, ds.author_id, Literal(row['author_id'], datatype=XSD.string)))
        g.add((tweet_uri, ds.screen_name, Literal(row['screen_name'], datatype=XSD.string)))
        ...
        g.add((tweet_uri, ds.user_followers_count, Literal(row['user_followers_count'], datatype=XSD.integer)))
        g.add((tweet_uri, ds.user_following_count, Literal(row['user_following_count'], datatype=XSD.integer)))
        g.add((tweet_uri, ds.verified, Literal(row['verified'], datatype=XSD.boolean)))
        ...
        g.add((tweet_uri, ds.mentionsBrand, brand_uri))
