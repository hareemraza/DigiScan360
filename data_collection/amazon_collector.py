...
def scrape_product_page(soup):
    """
    Extracts product details from a BeautifulSoup object representing a product page.

    Parameters:
    - soup (BeautifulSoup): A BeautifulSoup object of a product page.

    Returns:
    - dict: A dictionary containing product information such as title, overall rating,
            image URL, seller URL, price, number of reviews, rating histogram, and more reviews link.
    """
...
def scrape_products(product_links):
    """
    Scrapes product information from a list of product URLs.

    Parameters:
    - product_links (list): A list of URLs for products to be scraped.
...
def extract_review_information(review_tag):
    """
    Extracts review details from a tag representing a single review.

    Parameters:
    - review_tag (Tag): A BeautifulSoup Tag object of an individual review.

    Returns:
    - dict: A dictionary containing review information such as rating, title, review date,
            review body text, and the number of people who found the review helpful.
    """
    rating_tag = (
...
def scrape_reviews(products):
    """
    Scrapes reviews for a list of products.

    Parameters:
    - products (list): A list of dictionaries, each containing product information, including a link to more reviews.

    Returns:
    - list: A list of dictionaries, each representing a product's reviews, including review URL and the reviews themselves.
    """
    sessions = [
        requests.Session(),
        requests.Session(),
...
def crawl_and_scrape_amazon(query):
    """
    Coordinates the crawling and scraping of Amazon product links, product details, and reviews based on a search query.

    Parameters:
    - query (str): The search query to use for finding products on Amazon.

    Returns:
    - tuple: A tuple containing three elements; a list of all product links gathered, a list of dictionaries with
             products' details, and a list of dictionaries with products' reviews.
    """
